...
This meta-test:
  ...
  2. Verifies that the actual skip guard in test_mytube_49.py's
  ...
  3. Verifies that the actual skip message from test_mytube_49.py is
     descriptive — it instructs the user to set CDN_BASE_URL.
...
def _import_test_mytube_49():
    """Import test_mytube_49 module directly from its file path."""
    ...
    spec = importlib.util.spec_from_file_location("test_mytube_49", test_file)
...
class TestCDNBaseURLAbsence:
    ...
    def test_skip_guard_raises_skipped_with_descriptive_message(self, monkeypatch):
        """
        Invokes the REAL test_object_served_via_cdn_url from test_mytube_49.py
        with CDN_BASE_URL unset and a mock gcs_service.

        The guard at the top of the real test must raise pytest.skip.Exception
        and include 'CDN_BASE_URL' in the skip reason.
        """
        monkeypatch.delenv("CDN_BASE_URL", raising=False)
        config = GCSConfig()
        mock_service = MagicMock()

        module = _import_test_mytube_49()
        test_instance = module.TestHLSBucketProvisionedWithPublicAccess()

        with pytest.raises(pytest.skip.Exception) as exc_info:
            test_instance.test_object_served_via_cdn_url(mock_service, config)

        skip_reason = str(exc_info.value)
        assert "CDN_BASE_URL" in skip_reason, (
            f"Skip message in test_mytube_49.py must mention 'CDN_BASE_URL' so the "
            f"user knows what to set. Actual message: {skip_reason!r}"
        )

    def test_skip_message_is_descriptive(self, monkeypatch):
        """
        Invokes the REAL test_object_served_via_cdn_url from test_mytube_49.py
        with CDN_BASE_URL unset and asserts that the actual skip reason guides
        the user — it mentions CDN_BASE_URL and explains it needs to be set.
        """
        monkeypatch.delenv("CDN_BASE_URL", raising=False)
        config = GCSConfig()
        mock_service = MagicMock()
        ...
        assert "CDN_BASE_URL" in reason, (
            "Skip reason in test_mytube_49.py must mention the missing env var."
        )
        assert "not set" in reason.lower() or "unset" in reason.lower() or "set it" in reason.lower(), (
            "Skip reason in test_mytube_49.py should explain that CDN_BASE_URL "
            f"needs to be configured. Actual: {reason!r}"
        )
