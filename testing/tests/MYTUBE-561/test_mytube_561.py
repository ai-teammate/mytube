...
class TestMytube561HeaderBreakpoint:
    ...
    def test_static_shell_has_overflow_hidden(self) -> None:
        ...
        shell_block = re.search(r"\.shell\s*\{[^}]*\}", css, re.DOTALL)
        assert shell_block and "hidden" in shell_block.group(0), (
            "Expected overflow:hidden or overflow-x:hidden inside the .shell { } "
            "rule in globals.css. The .shell wrapper must clip any residual overflow "
            "from child elements."
        )
    ...
    def test_live_header_no_overflow_at_800px(
        self, config: WebConfig, playwright_browser: Browser
    ) -> None:
        """At 800px viewport, the header must not overflow horizontally."""
        self._assert_header_no_overflow(config, playwright_browser, width=800)

    def test_live_header_no_overflow_at_768px(
        self, config: WebConfig, playwright_browser: Browser
    ) -> None:
        """At exactly 768px — the breakpoint named in the ticket — the header
        must not overflow and elements must remain in a single row."""
        self._assert_header_no_overflow(config, playwright_browser, width=768)

    def test_live_header_no_overflow_at_760px(
        self, config: WebConfig, playwright_browser: Browser
    ) -> None:
        """At 760px — just below 768px — the header must still fit without
        stacking or overflowing."""
        self._assert_header_no_overflow(config, playwright_browser, width=760)

    # ──────────────────────────────────────────────────────────────────────────
    # Internal helpers
    # ──────────────────────────────────────────────────────────────────────────

    def _assert_header_no_overflow(
        self, config: WebConfig, browser: Browser, width: int
    ) -> None:
        ...
        Uses the SiteHeader page object for all DOM metric extraction.
        """
        ...
        page = browser.new_page(viewport={"width": width, "height": _HEIGHT})
        try:
            page.goto(
                config.home_url(),
            ...
            metrics = site_header.get_header_layout_metrics()
            ...
            assert row_count == 1, (
                ...
                f"rows at viewport width={width}px (expected exactly 1 row). "
